...
void draw() 
{
 ...
 // textFont(createFont("Arial bold",24));  
  ...
  if (numOfGraph ==1 )
  {
        graphRoll(Value1, VideoBuffer1, c_yellow, 1);
        fill(c_yellow);
  ...
  {
        graphRoll(Value1, VideoBuffer1, c_yellow, 1);
        graphRoll(Value2, VideoBuffer2, c_red, 2);
        fill(c_yellow);
        text(label1 + ": " + Value1, 180, 65);
        fill(c_red);
  ...
  if (numOfGraph == 3)
  {
        graphRoll(Value1, VideoBuffer1, c_yellow, 1);
        graphRoll(Value2, VideoBuffer2, c_red, 2);   
        graphRoll(Value3, VideoBuffer3, c_green, 3);
        fill(c_yellow);
        text(label1 + ": " + Value1, 180, 65);
        fill(c_red);
        text(label2 + ": " + Value2, 180, 90); 
  ...
  {
        //high pass filter for proximity so stays within bounds
        if(Value3 > 150) Value3 = 150;
        if(Value1 > 180) Value1 = 180;
        if(Value2 > 180) Value2 = 180;
        
        graphRoll( (Value1/360)*300 + 600, VideoBuffer1, c_yellow, 1); //pitch
        graphRoll( (Value2/360)*300 + 600, VideoBuffer2, c_red, 2);   //roll
        graphRoll(Value3 - 250, VideoBuffer3, c_green, 3); //proximity
    ...
    /*    graphRoll( (((Value4 - 65) / 40) * 280 + 150) + thermOffset   , VideoBuffer4, c_purple);
        ...
        graphRoll( ((Value4 * 10) + 150) + thermOffset   , VideoBuffer4, c_purple, 4);
        graphRoll( ((Value5 * 10) + 150) + thermOffset*2 , VideoBuffer5, c_cyan, 5);
        graphRoll( ((Value6 * 10) + 150) + thermOffset*3 , VideoBuffer6, c_orange, 6);
        graphRoll( ((Value7 * 10) + 150) + thermOffset*4 , VideoBuffer7, c_magenta, 7);
        graphRoll( ((Value8 * 10) + 150) , VideoBuffer8, c_burg, 8);
